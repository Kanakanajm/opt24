...
\begin{homeworkProblem}[1]
    \begin{homeworkSubsection}{1}
        \begin{enumerate}[(i)]
            ...
            {
                \color{red}
                \textbf{Note:} The function is convex when $p$ is even and not convex when $p$ is odd.
            }
    ...
    \begin{homeworkSubsection}{2}
        \begin{enumerate}[(i)]
            ...
            Similar to (i).\\
            {
                ...
                \] s.t.
                \[
                    |x_1| + |x_2| \leq 1 \text{ and } |y_1| + |y_2| \leq 1
                \]
                Consider $z = (1-\lambda)x + \lambda y$ where $\lambda \in [0, 1]$:
                \begin{align*}
                    ||z||_1 &= |(1-\lambda)x_1 + \lambda y_1| + |(1-\lambda)x_2 + \lambda y_2|\\
                    &\leq (1-\lambda)|x_1| + \lambda |y_1| + (1-\lambda)|x_2| + \lambda |y_2|\\
                    &= (1-\lambda)\underbrace{(|x_1| + |x_2|)}_{\leq 1} + \lambda \underbrace{(|y_1| + |y_2|)}_{\leq 1}
                    \tag*{interpolate between 2 numbers less and equal to 1}\\
                    &\leq 1
                \end{align*}
            }
        \end{enumerate}
    ...
    \begin{homeworkSubsection}{3}
        \begin{enumerate}[(i)]
            ...
            {
                \color{red}
                \textbf{Note:} This is not the definition of coercive. To prove coerciveness, we need to show that the function goes to infinity as the norm of the input goes to infinity. On the other hand, to prove non-coerciveness, we need to find a sequence of inputs such that the norms of the inputs tend to infinity but the function values do not.
                
                \[
                    x^4 + y^4 - x- y = \underbrace{(x^4 + y^4)}_{\underset{\sqrt{x^2 + y^2}\to \infty}{\to \infty}}(1 - \underbrace{\frac{x + y}{x^4 + y^4}}_{\underset{\sqrt{x^2 + y^2}\to \infty}{\to 0}})
                \]
                \item Consider the sequence $(x^{(k)}, y^{(k)}) := (k, -k)$. Clearly $\norm{(x^{(k)}, y^{(k)})}_2 = \sqrt{2k^2} \to \infty$ when $k \to \infty$.
                
                However, $f(x^{(k)}, y^{(k)}) = ak - bk + c = (a-b)k + c \to c$ when $k \to \infty$ if $a = b$ or $(a-b)k + c \to \infty^-$ if $a \leq b$.
            }
        \end{enumerate}
    \end{homeworkSubsection}
    \begin{homeworkSubsection}{4}
        ...
        {
            \color{red}
            \textbf{Note:} Both the tangent and normal cones are set of directions which have angles and magnitudes. In case of tangent and normal cones, their magnitudes don't matter. So, the correct answer should be:
            \begin{align*}
                T_C(\begin{pmatrix}
                    1 & 1
                \end{pmatrix}^\top) &= \left\{
                    \begin{pmatrix}
                        x_1 \\ x_2
                    \end{pmatrix} \in \mathbb{R}^2 \mid x_1 \leq 0, x_2 \leq 0
                \right\}\\
                N_C(\begin{pmatrix}
                    1 & 1
                \end{pmatrix}^\top) &= \left\{
                    \begin{pmatrix}
                        x_1 \\ x_2
                    \end{pmatrix} \in \mathbb{R}^2 \mid x_1 \geq 0, x_2 \geq 0
                \right\}\\
            \end{align*}
        }
    \end{homeworkSubsection}
...
\begin{homeworkProblem}[2]
    \begin{homeworkSubsection}{(A)}
        ...
        {
            \color{red}
            \textbf{Corrected Answer (Direction)}\\
            For a quadratic function $f(x) = \frac{1}{2}x^\top Qx + b^\top x$. Suppose $Q$ is symmetric and positive definite. We can decompose $Q$ as $Q = LL^\top$ where $L$ is a lower triangular matrix. Let $y = L^\top x$, we reformulate the optimization problem to solve for $y$:
            \[
                \min_{y} \frac{1}{2}y^\top y + b^\top L^{-\top}y
            \]
            Perform steepest descent on $y$:
            \[
                y^{(k+1)} = y^{(k)} - \alpha \nabla f(y^{(k)}) = y^{(k)} - \alpha (y^{(k)} + L^{-1}b)
            \]
        }
    \end{homeworkSubsection}
