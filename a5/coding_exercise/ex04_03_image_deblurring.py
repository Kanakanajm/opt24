...
if run_exp == 0:
    ...
    run_cg_1 = 1     # Conjugate Gradient Method - Jacobi Preconditioner
    run_cg_2 = 1     # Conjugate Gradient Method - Cholesky Preconditioner

    run_newton = 1     # Newton Method

if run_exp == 1:
    # Experiment 1: To compare the behavior of the Newton method vs Gradient Descent
    # turn algorithms to be run on or off
...
if run_cg_1:
    ...
    print('')
    print('********************************************************************************')
    print('*** Conjugate Gradient with Jacobi Pre-conditioner ***')
    print('**************************')
    ...
    ts.append(output['seq_time'])
    cols.append((0.1, 0.5, 0.5, 1))
    legs.append('CG-JACOBI')
    nams.append('CG-JACOBI')

    if run_exp == 0:
        mpimg.imsave("figures/deblurred_image_cg_jacobi.png",
                     output['sol'].reshape(ny, nx), cmap=plt.cm.gray)
...
if run_cg_2:
    ...
    print('')
    print('********************************************************************************')
    print('*** Conjugate Gradient with Cholesky Pre-conditioner ***')
    print('**************************')
    ...
    ts.append(output['seq_time'])
    cols.append((0.8, 0.6, 0, 1))
    legs.append('CG-CHOLESKY')
    nams.append('CG-CHOLESKY')

    if run_exp == 0:
        mpimg.imsave("figures/deblurred_image_cg_cholesky.png",
                     output['sol'].reshape(ny, nx), cmap=plt.cm.gray)
...
if run_newton:
    maxiter_newton = 1 # explicitly setting maximum number of iterations to 1
    ...
    output = newton(model, options, maxiter_newton, check)
    xs.append(output['sol'])
    rs.append(output['seq_res'])
    ts.append(output['seq_time'])
    ...
    if run_exp == 0:
        mpimg.imsave("figures/deblurred_image_newton_0.png",
                     output['sol'].reshape(ny, nx), cmap=plt.cm.gray)
...
if run_exp == 0:
    ...
    # TODO: Compare Newton method with algorithm of your choice.
    # TODO: Below we use 5 as a placeholder, you need to change it with whatever algorithm you seem fit.
    fig = plt.figure()
    j = 0
    for i in [5, 6]:
        plt.loglog(rs[i], '-', color=cols[i], linewidth=2)
    legs_temp = [legs[i] for i in (5, 6)]

    plt.legend(legs_temp)
    plt.xlabel('Iterations')
    plt.ylabel('Function value')
    plt.title('Function vs Iterations')
